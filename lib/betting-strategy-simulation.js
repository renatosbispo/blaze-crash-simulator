...
const desiredStrategyIndex = 0;
const desiredSampleIndex = 1;
...
const sample = loadProgramData(CRASH_POINTS_HISTORY_SAMPLES_FILE_PATH)[
  desiredSampleIndex
];
...
export default function simulateStrategy() {
  ...
  for (let i = 0; i < numberOfRuns && crashPointIndex > -1; i++) {
    const runStopOnProfit =
      stopOnProfit < 1 ? stopOnProfit * currentBalance : stopOnProfit;
    const runStopOnLoss =
      stopOnLoss < 1 ? stopOnLoss * currentBalance : stopOnLoss;
    const runBetValue =
