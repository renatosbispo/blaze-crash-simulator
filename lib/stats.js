...
const crashPointValuesOfInterest = generateCrashPointValuesOfInterest(
  1.4,
  3.5,
  0.05
);
...
function getCrashPointValuesProbabilities(sample) {
  const crashPointValuesProbabilities = crashPointValuesOfInterest.map(
    (crashPointValue) => {
      const probability =
        (sample.reduce((count, { value }) => {
...
console.log(
  _.orderBy(
    getCrashPointValuesProbabilities(samplesRaw[5]),
    ['probability'],
    ['desc']
  )
);
